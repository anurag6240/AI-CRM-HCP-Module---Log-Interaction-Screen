...
1. **Dual-Pane Interactive Layout**:
...
5. Start the FastAPI server:
   ...
   ```
   The backend API will run at [http://localhost:8000](http://localhost:8000) and Swagger API docs at [http://localhost:8000/docs](http://localhost:8000/docs).
...
3. Run the Vite frontend:
   ...
   npm run dev
   ```
   Open your browser and navigate to [http://localhost:5173](http://localhost:5173). It will communicate with the FastAPI server on port 8000.

---

## REST API Routes

### 1. Interactions Endpoints
...
1. **Log Interaction Tool**: Parses raw unstructured text to identify entities (doctor name, attendees, clinical topics, outcomes) and compiles them into a structured JSON payload conforming to the database schema.
2. **Edit Interaction Tool**: Context-aware editor that compares raw chat inputs alongside existing form states to perform surgical overrides and field updates.
3. **Sentiment Analyzer Tool**: Evaluates the tone and notes, classifying the doctor's sentiment as **Positive**, **Neutral**, or **Negative**.
4. **Material & Sample Locator Tool**: Scans text to identify physical materials distributed (brochures, trial data) and starter drug samples.
5. **Follow-Up Action Generator Tool**: Reviews the meeting outcomes to generate targeted next steps (e.g. scheduling roundtable leaders, coordinating medical inquiries).
